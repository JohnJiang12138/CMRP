...
from DropEdge.src.utils import data_loader, sparse_mx_to_torch_sparse_tensor
from normalization import fetch_normalization

class Sampler:
    """Sampling the input graph data."""
    def __init__(self, dataset, data_path="DropEdge/data", task_type="full"):
    ...
    def randomedge_sampler(self, percent, normalization, cuda, selected_nodes=None):
        """
        ...
        if selected_nodes is not None:
            perm = perm[~np.isin(perm,selected_nodes)]
            perm = np.concatenate([perm[:preserve_nnz], selected_nodes], axis=0)
        else:
