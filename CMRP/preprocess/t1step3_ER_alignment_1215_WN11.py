...
def embedding_based_search(query_embeddings, entityid2embeds, threshold=0.5):
    ...
    # 接着，将该数组保存为.pkl文件
    with open('../REINFORCE/WN11_entity_similarities.pkl', 'wb') as f:
        pickle.dump(entitiy_similarities, f)

    # 加载保存的.pkl文件
    with open('../REINFORCE/WN11_entity_similarities.pkl', 'rb') as f:
...
def embedding_based_search_rel(query_embeddings, entityid2embeds, threshold=0.5):
    # 将 entityid2embeds 中的 numpy.ndarray 转换为 Tensor，并确保它们在正确的设备上
    # pdb.set_trace()
...
def save_entity_and_relation_embeddings(cls=None):
    ...
    if cls == 'entity':
        ...
        pickle.dump(embeds_dict, open('WN11_entity_embs.pkl', 'wb'))
    elif cls == 'relation':
        ...
        pickle.dump(embeds_dict, open('WN11_relation_embs.pkl', 'wb'))
    else:
...
with Timing("若没有加载实体和关系嵌入，则加载它们"): 
    text2entityid_file = '../knowledge_graph_tasks/embedding_based/benchmarks/WN11/text2entityid.txt'
    text2relationid_file = '../knowledge_graph_tasks/embedding_based/benchmarks/WN11/text2relationid.txt'
    ...
    if not os.path.exists('WN11_entity_embs.pkl'):
        save_entity_and_relation_embeddings(cls='entity')
    if not os.path.exists('WN11_relation_embs.pkl'):
        save_entity_and_relation_embeddings(cls='relation')


    entityid2embeds = pickle.load(open('WN11_entity_embs.pkl', 'rb'))
    relationid2embeds = pickle.load(open('WN11_relation_embs.pkl', 'rb'))
...
with Timing("执行基于实体嵌入的搜索"): 
    ...
    most_similar_entity_ids = embedding_based_search(entity_embeddings, entityid2embeds,0)

with Timing("执行基于关系嵌入的搜索"): 
    # 执行基于关系嵌入的搜索
    print('searching for relations...')
    most_similar_relation_ids = embedding_based_search_rel(relation_embeddings, relationid2embeds,0)
...
def hypergraph_nodes_and_relation_mapping_to_KG(data, entity_target_pairs, relation_target_pairs):
    ...
    pickle.dump(hg_entities, open('WN11_hg_entities.pkl', 'wb'))
    pickle.dump(hg_relations, open('WN11_hg_relations.pkl', 'wb'))
