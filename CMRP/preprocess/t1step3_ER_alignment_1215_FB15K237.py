...
def embedding_based_search(query_embeddings, entityid2embeds, threshold=0.5):
    ...
    # 接着，将该数组保存为.pkl文件
    with open('../REINFORCE/FB15K237_entity_similarities.pkl', 'wb') as f:
        pickle.dump(entitiy_similarities, f)

    # 加载保存的.pkl文件
    with open('../REINFORCE/FB15K237_entity_similarities.pkl', 'rb') as f:
...
with Timing("若没有加载实体和关系嵌入，则加载它们"): 
    ...
    if not os.path.exists('FB15K237_entity_embs.pkl'):
        save_entity_and_relation_embeddings(cls='entity')
    if not os.path.exists('FB15K237_relation_embs.pkl'):
        save_entity_and_relation_embeddings(cls='relation')


    entityid2embeds = pickle.load(open('FB15K237_entity_embs.pkl', 'rb'))
    relationid2embeds = pickle.load(open('FB15K237_relation_embs.pkl', 'rb'))
...
def hypergraph_nodes_and_relation_mapping_to_KG(data, entity_target_pairs, relation_target_pairs):
    ...
    pickle.dump(hg_entities, open('FB15K237_hg_entities.pkl', 'wb'))
    pickle.dump(hg_relations, open('FB15K237_hg_relations.pkl', 'wb'))
