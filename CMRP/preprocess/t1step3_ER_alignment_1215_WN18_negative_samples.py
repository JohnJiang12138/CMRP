...
def embedding_based_search(query_embeddings, entityid2embeds, threshold=0.1):
    ...
    # 接着，将该数组保存为.pkl文件
    with open('../REINFORCE/WN18_entity_similarities_negative_samples.pkl', 'wb') as f:
        pickle.dump(entitiy_similarities, f)

    # 加载保存的.pkl文件
    with open('../REINFORCE/WN18_entity_similarities_negative_samples.pkl', 'rb') as f:
...
def save_entity_and_relation_embeddings(cls=None):
    ...
    if cls == 'entity':
        ...
        pickle.dump(embeds_dict, open('WN18_entity_embs_negative_samples.pkl', 'wb'))
    elif cls == 'relation':
        ...
        pickle.dump(embeds_dict, open('WN18_relation_embs_negative_samples.pkl', 'wb'))
    else:
...
#         pickle.dump(embeds_dict, open('WN18_entity_embs.pkl', 'wb'))
#     elif cls == 'relation':
...
#         pickle.dump(embeds_dict, open('WN18_relation_embs.pkl', 'wb'))
#     else:
#         raise ValueError('cls value is wrong')

with Timing("若没有加载实体和关系嵌入，则加载它们"): 
    text2entityid_file = '../knowledge_graph_tasks/embedding_based/benchmarks/WN18/text2entityid.txt'
    text2relationid_file = '../knowledge_graph_tasks/embedding_based/benchmarks/WN18/text2relationid.txt'
    ...
    if not os.path.exists('WN18_entity_embs_negative_samples.pkl'):
        save_entity_and_relation_embeddings(cls='entity')
    if not os.path.exists('WN18_relation_embs_negative_samples.pkl'):
        save_entity_and_relation_embeddings(cls='relation')


    entityid2embeds = pickle.load(open('WN18_entity_embs_negative_samples.pkl', 'rb'))
    relationid2embeds = pickle.load(open('WN18_relation_embs_negative_samples.pkl', 'rb'))
...
def hypergraph_nodes_and_relation_mapping_to_KG(data, entity_target_pairs, relation_target_pairs):
    ...
    pickle.dump(hg_entities, open('WN18_hg_entities_negative_samples.pkl', 'wb'))
    pickle.dump(hg_relations, open('WN18_hg_relations_negative_samples.pkl', 'wb'))
