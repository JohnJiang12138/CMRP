...
def embedding_based_search(query_embeddings, entityid2embeds, threshold=0.5):
    ...
    # 接着，将该数组保存为.pkl文件
    with open('../REINFORCE/FB13_entity_similarities.pkl', 'wb') as f:
        pickle.dump(entitiy_similarities, f)

    # 加载保存的.pkl文件
    with open('../REINFORCE/FB13_entity_similarities.pkl', 'rb') as f:
...
def save_entity_and_relation_embeddings(cls=None):
    ...
    if cls == 'entity':
        ...
        pickle.dump(embeds_dict, open('FB13_entity_embs.pkl', 'wb'))
    elif cls == 'relation':
        ...
        pickle.dump(embeds_dict, open('FB13_relation_embs.pkl', 'wb'))
    else:
...
with Timing("若没有加载实体和关系嵌入，则加载它们"): 
    text2entityid_file = '../knowledge_graph_tasks/embedding_based/benchmarks/FB13/text2entityid.txt'
    text2relationid_file = '../knowledge_graph_tasks/embedding_based/benchmarks/FB13/text2relationid.txt'
    ...
    if not os.path.exists('FB13_entity_embs.pkl'):
        save_entity_and_relation_embeddings(cls='entity')
    if not os.path.exists('FB13_relation_embs.pkl'):
        save_entity_and_relation_embeddings(cls='relation')


    entityid2embeds = pickle.load(open('FB13_entity_embs.pkl', 'rb'))
    relationid2embeds = pickle.load(open('FB13_relation_embs.pkl', 'rb'))
...
def hypergraph_nodes_and_relation_mapping_to_KG(data, entity_target_pairs, relation_target_pairs):
    ...
    pickle.dump(hg_entities, open('FB13_hg_entities.pkl', 'wb'))
    pickle.dump(hg_relations, open('FB13_hg_relations.pkl', 'wb'))
