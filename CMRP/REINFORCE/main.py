...
import trainer_kg, trainer_llm, trainer_nc, trainer_gc, trainer_nc_prog,trainer_kge
sys.path.append('../KGE_DURA')
from KGE_DURA.code.process_datasets import prepare_dataset
from joint_train import joint_train

# os.environ["http_proxy"] = "http://agent.baidu.com:8118"
# os.environ["https_proxy"] = "http://agent.baidu.com:8118"
...
def main(args):
    ...
    if args.graph_task == 'kg':
    ...
    elif args.graph_task == 'kge':
        if args.kge == 'DURA':
            path = f'../KGE_DURA/src_data/{datasetname}/'
            replace_file_content('org_train', 'train',path)
            prepare_dataset(path=f'../KGE_DURA/src_data/{datasetname}/',name = datasetname, DATA_PATH = f'../KGE_DURA/data')
        elif args.kge == 'HAKE':
            path = f'../KGE_HAKE/data/{datasetname}/'
            replace_file_content('org_train.txt', 'train.txt',path)
        # 此处需要将RL过后的数据集恢复为原始的数据集，从备份中恢复，仅需对train进行

        #hyper_edges 对于KGE系列数据集 编号对齐后是 通用的
        hyper_edges = pickle.load(open(f'../preprocess/{datasetname}_new_hg_triplets.pkl', 'rb'))
        hyper_edges_ns = pickle.load(open(f'../preprocess/{datasetname}_new_hg_triplets_negative_samples.pkl', 'rb'))
        graph_trainee = trainer_kge.Trainer(args, hyper_edges, hyper_edges_ns) #注意是trainer_KGE
        ...
        path = f'../ProG/dataset/{datasetname}/raw/'
        replace_file_content(f'ind.{datasetname}.graph_org', f'ind.{datasetname}.graph',path)
        if(args.nc_type=='ProG'):
            graph_trainee = trainer_nc_prog.Trainer(args, hypergraph)
        else:
    ...
    # if args.llm_task == 'qa':
    #     # llm_KG_triplets = load_processed_triplets(f'../LLM_tasks/{datasetname}_processed_triplets.txt')
    #     llm_KG_triplets = pickle.load(open(f'../LLM_tasks/{datasetname}_all_subgraphs_dict.pkl', 'rb'))
    #     # print(llm_KG_triplets.shape)
    #     # print(llm_KG_triplets[0])
    #     # print(llm_KG_triplets[0][0])
    #     # print(llm_KG_triplets[0][0][0])
    #     # print(llm_KG_triplets)
    #     # llm_KG_triplets = pickle.load(open(f'../LLM_tasks/{datasetname}_processed_triplets.txt', 'rb'))
